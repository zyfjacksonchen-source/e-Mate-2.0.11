...
describe('macOS release artifact verification', () => {
  it('isolates forced-stop architecture probes from each other', () => {
    ...
    expect(source).toContain('Startup performance is a paired installed-artifact')
    expect(source).not.toContain('RELEASE_STARTUP_CEILING_MS')
    expect(source).not.toContain('15-second startup ceiling')
