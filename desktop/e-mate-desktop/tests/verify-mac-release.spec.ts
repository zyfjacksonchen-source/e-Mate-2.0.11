...
describe('macOS release artifact verification', () => {
  it('isolates forced-stop architecture probes from each other', () => {
    ...
    expect(source).toContain("join(userData, '.release-health-failure')")
    expect(source).toContain('packaged application failed before ${outcome}')
    expect(source).toContain('const RELEASE_STARTUP_CEILING_MS = 15_000')
    expect(source).toContain('if (elapsedMs > RELEASE_STARTUP_CEILING_MS)')
    expect(source).toContain('exceeded the 15-second startup ceiling')
    expect(source).toContain('const RELEASE_SHUTDOWN_TIMEOUT_MS = 10_000')
    expect(source).toContain("terminateProcess(-processGroup)")
    expect(source).toContain("terminateProbeChrome(join(root, `dsh-${arch}`, 'runtime', 'cdp-chrome'))")
    expect(source).toContain('`--user-data-dir=${userDataDir}`')
    expect(source).not.toContain('RELEASE_HEALTH_TIMEOUT_MS')
    expect(source).not.toContain('180_000')
