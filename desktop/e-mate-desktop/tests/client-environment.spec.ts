...
describe('advanced desktop layout', () => {
  it('owns native caption geometry without targeting feature headers', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\.dshDesktopFrame \{[^}]*background:\s*transparent;/)
      expect(css).toMatch(/\.dshDesktopSidebarSurface\s*\{[^}]*--dsw-specific-sidebar-fill:\s*transparent;[^}]*background:\s*transparent;/)
      ...
      expect(css).toMatch(/\.dshDesktopConversationSurface \{[^}]*background:\s*var\(--dsw-alias-bg-base\);/)
      expect(css).toMatch(/\.dshDesktopDetailsSurface \{[^}]*background:\s*var\(--dsw-alias-bg-base\);/)
      expect(css).not.toContain('--emate-color-workspace')
      ...
      expect(css).toMatch(/\.dshDesktopWindowsCaptionRow \{[^}]*background:\s*var\(--dsw-alias-bg-base\);/)
