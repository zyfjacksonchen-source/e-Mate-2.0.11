...
describe('desktop release manifest', () => {
  it('binds both immutable R2 artifacts to bytes and SHA-256', async () => {
    ...
    const macArtifact = join(artifacts, 'e-Mate-2.0.7-mac-universal.dmg')
    const windowsArtifact = join(artifacts, 'e-Mate-2.0.7-win-x64-Setup.exe')
    ...
    expect(manifest).toEqual({
      ...
      artifacts: {
        darwin: {
          url: `https://pub-ada3f610c0234a76838f4e19fe2bb25e.r2.dev/desktop/releases/v2.0.7/${commit}/e-Mate-2.0.7-mac-universal.dmg`,
          bytes: macBytes.byteLength,
          sha256: createHash('sha256').update(macBytes).digest('hex'),
        },
        win32: {
          url: `https://pub-ada3f610c0234a76838f4e19fe2bb25e.r2.dev/desktop/releases/v2.0.7/${commit}/e-Mate-2.0.7-win-x64-Setup.exe`,
  ...
  it('rejects mutable or misnamed release inputs', async () => {
    ...
    const windowsArtifact = join(root, 'e-Mate-2.0.7-win-x64-Setup.exe')
