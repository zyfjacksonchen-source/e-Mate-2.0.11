...
describe('pinned assisted-NSIS atomic update seam', () => {
  ...
  it('leaves fresh/manual install and builder-owned uninstaller generation on the upstream path', () => {
    ...
    expect(include.indexOf('SetOutPath "$PLUGINSDIR"'))
      .toBeGreaterThan(include.indexOf('!macro customUpdateInstallApply'))
    expect(include.indexOf('SetOutPath "$PLUGINSDIR"'))
      .toBeLessThan(include.indexOf('emateRunUpdateTransaction "Apply"'))
