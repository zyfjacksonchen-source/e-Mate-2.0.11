...
- The 2.0.10 formal macOS package is ad-hoc signed, not Developer ID signed or notarized. Fresh installation and the one-time 2.0.9-to-2.0.10 handoff can therefore require the documented per-app quarantine removal. Installed 2.0.10 and later updates replace the app automatically after exact artifact and bundle validation. Windows local `dist:win` remains unsigned; publisher identity, SmartScreen reputation, and native upgrade testing remain release gates.
