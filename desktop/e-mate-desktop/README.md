...
Packaged Base v7 macOS and Windows applications query the signed R2 manifest at `https://pub-ada3f610c0234a76838f4e19fe2bb25e.r2.dev/desktop/signed/latest.json` 60 seconds after startup and every six hours after a completed check. The existing updater accepts only a manifest whose complete canonical body verifies with an Ed25519 key from the packaged Base contract before it compares versions or selects an artifact. Each no-cache request has a 15-second deadline and shares one in-flight operation with the **Check for Updates…** tray command. Background network, HTTP, timeout, invalid-response, equal-version, older-version, and invalid-signature outcomes are silent. Development, unpackaged, and Linux launches do not download an installer.

Choosing **Update and Restart** first rechecks that the advertised version is unchanged, then makes the first request to the platform's fixed counted download endpoint. e-Mate follows the service redirect through Electron networking, streams at most 1 GiB into a private versioned user-data directory, and verifies the manifest byte count, SHA-256, and DMG or Windows PE container before installation. Starting with an installed 2.0.10 app, macOS also verifies the mounted bundle identity, version, universal executable and native inventory, and complete ad-hoc code seal. A detached helper first selects the exact request protocol, persists its matching ready handshake and stages only at canonical `Applications/e-Mate.app`; it swaps only after orderly Cordis teardown writes `shutdown-ready`, and keeps the previous version until the new renderer reports healthy. The 2.0.14 Helper accepts the exact published 2.0.12 request through its bounded legacy swap/ACK path, while 2.0.13 uses the bound pending/IPC path. Failure restores, relaunches, and reports the previous bundle; renderer health is the legacy commit boundary and the bound path retains its stronger durable confirmation. The first 2.0.9-to-2.0.10 upgrade still uses 2.0.9's old manual DMG handoff because an installed old binary cannot acquire new updater logic remotely. On Windows, the same confirmation launches the prepared NSIS installer before requesting orderly teardown; there is no second application confirmation. No path silently elevates, disables Gatekeeper, or claims publisher identity.

The published 2.0.12 `desktop/latest.json` stays frozen as an equal-version compatibility tombstone because its installed parser cannot verify the new signature. The protected external admission owner may publish one identical signed manifest to the version-specific manual path and the active signed pointer only after exact-byte native acceptance and public readback. Base v7 continues through the same updater, download verifier, and platform transaction; there is no second updater or Redis version switch.
