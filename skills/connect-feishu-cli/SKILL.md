...
3. Before any authorization command, run `pnpm dlx @larksuite/cli@1.0.88 auth status --json --verify`. Reuse the existing device-global profile whenever the required identity reports `available: true` and `verified: true`, including user status `needs_refresh`: execute the requested user API operation normally and let the official CLI refresh its keychain token. In this reusable state, do not run `config init`, `auth login`, or another authorization scan.
