...
1. Call `skill_install` with source `riba2534/feishu-cli`, skill `feishu-cli-platform`, and the scope appropriate to the user's request. The source is already known and trusted, so do not depend on a second Skill Hub search or substitute a webhook-only Skill. After installation, load that Skill and follow its current CLI installation and security instructions.
...
6. For the requested business domain, install the matching `feishu-cli-*` Skill from `riba2534/feishu-cli`, load it, and complete one real requested operation. Use documents for docs, messaging for chats, storage for Drive/Wiki, data for Sheet/Bitable, work for calendar/tasks/approval, and the corresponding current domain Skill for mail, meetings, or visual work. Use `skill_find` only when the repository's current domain Skill name is unknown.
