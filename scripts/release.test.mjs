...
test('GitHub release packs once and validates the same tarball on three platforms', () => {
  ...
  assert.doesNotMatch(desktopReleaseSource, /build:harness|pnpm test|build:sdk|dist:win|dist:mac/u)
...
test('keeps the checked-in 2.0.15 page bound to the exact updater and unsigned publication warning', async t => {
  ...
  assert.match(page, /2\.0\.13.*手动/u)
  assert.match(page, /既有同版本旧 2\.0\.15.*手动/u)
  assert.match(macGuide, /2\.0\.13.*手动/u)
  ...
  assert.match(page, /从本次新 2\.0\.15 起，后续版本可在应用内完成更新/u)
  assert.match(page, /\/ecorex-agent\/admin\//u)
  assert.doesNotMatch(page, /2\.0\.13 的用户可在应用内确认更新/u)
  assert.match(macGuide, /全新安装或手动替换 2\.0\.15/u)
  assert.match(macGuide, /2\.0\.13 和既有同版本旧 2\.0\.15 需在本页下载后手动替换/u)
