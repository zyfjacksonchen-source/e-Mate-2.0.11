...
import { PACKAGE_NAME, releaseSource, VERSION } from './release-source.mjs'
...
export async function generateEvidence(directory, outputDirectory, sourceCommit = assertEvidenceSource()) {
  ...
  const manifest = {
    ...
    evidence: ['SHA256SUMS', `e-mate-${VERSION}.spdx.json`, 'THIRD_PARTY_LICENSES.txt', 'EVIDENCE_SHA256SUMS'],
  }
  ...
  const files = {
    ...
    [`e-mate-${VERSION}.spdx.json`]: `${JSON.stringify(spdx, null, 2)}\n`,
