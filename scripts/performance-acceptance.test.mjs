...
test('wires the one-shot producer before the existing exact-85 consumer', async () => {
  ...
  assert.match(workflow, /macos_publication_mode:[\s\S]*?required: true[\s\S]*?type: choice[\s\S]*?options: \[unsigned, signed\]/u)
  assert.match(workflow, /macos_signer_run_id:[\s\S]*?required: false[\s\S]*?type: string/u)
  assert.match(workflow, /unsigned\) test -z "\$MACOS_SIGNER_RUN_ID"/u)
  assert.match(workflow, /const macosBuildRunId = macosPublicationMode === 'unsigned'[\s\S]*?\? ciRunId[\s\S]*?: macosPublicationMode === 'signed' \? macosSignerRunId : undefined/u)
  assert.match(workflow, /candidate\.artifacts\.darwin\.build_run_id !== macosBuildRunId/u)
  assert.match(workflow, /candidate\.artifacts\.win32\.build_run_id !== ciRunId/u)
  assert.match(workflow, /macos-signer-run-id: \$\{\{ inputs\.macos_publication_mode == 'signed' && inputs\.macos_signer_run_id \|\| '' \}\}/u)
  assert.match(workflow, /e-mate-desktop-publication\/performance@e45c3b9d1bec366ab306203574d0a7a724d7f123/u)
  assert.doesNotMatch(workflow, /e-mate-desktop-publication\/performance@(?:1f46dbe1d34ac558cb2e91b52b2ab30f7eab6c4c|cd7d223692b51e4e7a53db5759e1c2a9811febd0)/u)
})
