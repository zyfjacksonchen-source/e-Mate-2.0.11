...
test('keeps current e-Mate source owners on 2.0.17', () => {
  ...
  for (const path of [
    ...
    ...pluginManifests,
  ]) assert.equal(json(path).version, '2.0.17', path)
  ...
  assert.equal(contract.runtime_imports['@e-mate/desktop/vision-toolkit'], '2.0.17')
...
test('keeps public download pointers on 2.0.16', () => {
  for (const path of ['deploy/download-page/index.html', 'deploy/download-page/install-macos.html']) {
    ...
    assert.doesNotMatch(html, /data-desktop-version="2\.0\.17"/u, path)
  }
})
