...
export async function createGithubArtifactProvenance(options) {
  ...
  const ciJobs = await metadata(options.metadata, 'ci-jobs.json')
  successfulJob(ciJobs, 'CI admission', 'CI')
  successfulJob(ciJobs, 'Node 24 / target contracts and unit tests', 'CI')
