...
function request(path, options = {}, userId = 'user-1') {
...
async function direct(env, path, options, userId) {
  const originalFetch = globalThis.fetch
  globalThis.fetch = activeSession
  try { return await worker.fetch(request(path, options, userId), env) } finally { globalThis.fetch = originalFetch }
}
...
test('publishes, discovers, downloads, installs, reconciles, and tombstones one immutable Skill', async () => {
  ...
  assert.deepEqual(await completed.json(), { schema_version: 1, status: 'installed' })
  ...
  assert.deepEqual(await reconciled.json(), { schema_version: 1, status: 'installed' })
  ...
  assert.deepEqual((await owned.json()).items, [card])
