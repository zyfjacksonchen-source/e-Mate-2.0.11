...
`office-skills` supplies four clean-room Skills for documents, PDF, spreadsheets and presentations. It does not ship the deleted Python Worker or claim local Office-format execution until an accepted host toolchain closes the action. Internet search is the pinned rc.7 DeepSeek provider already mounted by `@deepseek-ai/dsh-base`, with its credential reference projected by e-Mate model policy; no Search MCP component or Session-local Skill participates. `genui` and `better-sidebar` contribute target UI slots. The pinned Harness remains the sole owner of native Subagent services.
