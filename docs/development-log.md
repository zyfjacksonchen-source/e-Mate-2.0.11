...
- 三个 manifest 现只描述实际 emitted runtime imports；全局 Base v6 runtime ABI union 和 Base id 均未变化。唯一组件 runner 在 portable build 或带精确 `EMATE_COMPONENT_TARGET` 的平台 build 后立即从 `files` 闭包提取真实外部 import，并要求与 `base_imports` 排序全等；没有目标 Python/runtime 的通用检查构建不冒充平台发布字节，三套 Base compatibility Jobs 负责各自目标，发布 emit 复用同一验证函数。Glass、Skill Hub 和 Vision 三目标共五个实际 payload 均已重新 emit 成功；旧 SHA 上仍运行的 CI、Desktop 和 npm carrier 已取消，不得复用为正式候选。Profile workflow 的 bootstrap 说明也同步为 accepted 2.0.12 Base。
