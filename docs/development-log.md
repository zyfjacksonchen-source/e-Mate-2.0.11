...
- 唯一组件 runner 现在在 portable build 或带精确 `EMATE_COMPONENT_TARGET` 的平台 build 后立即从 `files` 闭包提取真实外部 import，并要求与 `base_imports` 排序全等；没有目标 Python/runtime 的通用检查构建不冒充平台发布字节，三套 Base compatibility Jobs 负责各自目标，发布 emit 复用同一验证函数。PR clean target CI 随即证明本机残留 `.build/lib` 给出的 Vision 缩减结论不可信：darwin-arm64、darwin-x64 与 win32-x64 的新鲜目标构建都真实 emitted `dsh-credentials`、`dsh-settings`、`dsh-tools`、`schemastery`，所以 Vision 恢复原完整声明，不能通过放宽比较掩盖候选字节差异。Glass 与 Skill Hub 的修正、全局 Base v6 runtime ABI union 和 Base id 均保持不变；旧 SHA 上的 CI、Desktop、npm carrier 和本机 stale-lib payload 全部作废。Profile workflow 的 bootstrap 说明同步为 accepted 2.0.12 Base。
