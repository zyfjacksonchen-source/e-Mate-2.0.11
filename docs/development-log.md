...
## 2026-08-31 · 2.0.15 T25R14 Windows Profile lifecycle shell

- Baseline and evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T25R14-profile-shell`, branch `fix/2.0.15-T25R14-profile-shell`, exact parent `fadb25a4aae5d568db6b48d29361c4d0f8844dff`. The Windows batch sweep reached the sole local-flow `profile-build` stage after component ABI closure, where the Profile package's POSIX relative `tsdown` shim was handed to native `cmd.exe` and failed as `'..' is not recognized`. This is a local build-time boundary, so Sentry is `NO_EVIDENCE`.
- Minimal root fix: only the existing Profile main-package `run build` invocation now enables pnpm 11.7.0's native `--config.shell-emulator=true`. Root and Harness installs remain byte-for-byte native, the pinned lifecycle carrier remains authoritative, and no package script, dependency, lock, global shell setting or second builder changed.
- RED/GREEN and ceiling: the focused contract first failed on the old Profile argv, then passed with native install argv and the portable Profile build argv locked together; the complete local-flow suite passes `47/47`, both changed scripts parse, the change-impact contract and `git diff --check` pass. No candidate, package build, installer, Windows command, CI, R2, signing, installation, update, rollback or publication ran; the release Owner must continue the same batch sweep before one final candidate build.

## 2026-08-31 · 2.0.15 T25R15 release tar drive-letter portability

- Baseline and evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T25R15-tar-paths`, branch `fix/2.0.15-T25R15-tar-paths`, exact parent `fadb25a4aae5d568db6b48d29361c4d0f8844dff`. The Windows batch release suite reached Git for Windows GNU tar, where all four verifier reads/extractions and all three fixture creations passed an absolute `C:\...` archive argument; GNU tar interpreted the drive-letter colon as a remote archive. This is a local build-time boundary, so Sentry is `NO_EVIDENCE`.
- Minimal root fix: one private release helper resolves the archive once, runs the system tar from its containing directory, and passes only its basename; list, verbose list, entry read and extraction retain their existing flags, extraction destination and fail-closed entry/link checks. The three test fixture creators use the same cwd plus relative archive-name shape. No `--force-local`, platform branch, dependency or replacement tar implementation was added.
- RED/GREEN and ceiling: the focused contract first rejected the old four direct product invocations, then passed while locking one helper plus four callers. The complete Mac release Owner suite passes `17/17` using an existing read-only dependency closure after the fresh worktree's environment-only `yaml` miss; both scripts parse, the change-impact contract/classification and `git diff --check` pass. No candidate, installer, Windows command, CI, R2, signing, installation, update, rollback or publication ran; Windows GREEN remains for the main batch sweep.

## 2026-08-31 · 2.0.15 T25R16 Windows Python executable selection

- Baseline and evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T25R16-python-selector`, branch `fix/2.0.15-T25R16-python-selector`, exact parent `f4c4c81e934a8c6713bc46f0075fec9fd8b9384a`. The formal Windows `_platform-build` reached `native-runtime-inputs`, where local-flow invoked bare `python`; PATH exposed only the non-executable WindowsApps alias and the command returned `9009`. This is a local build-time boundary, so Sentry is `NO_EVIDENCE`.
- Minimal root fix: the sole local-flow Python selector now verifies Python 3 before use. An existing non-empty `EMATE_BUILD_PYTHON` exact command has first priority and is invoked directly without shell parsing; an empty, padded, unavailable or non-Python-3 explicit value fails closed rather than silently selecting another interpreter. Without that Owner input, Windows tries the native launcher as `py -3` then a successfully executable `python`, while macOS retains verified `python3`. The exact selected prefix is carried into the existing `prepare-wheels.py` invocation, aligning local-flow with the target-component caller's existing exact CPython contract.
- RED/GREEN and ceiling: focused RED proved both the absent selector and the still-unwired formal stage. Focused GREEN covers `py -3`, a real `python` fallback, missing launcher plus WindowsApps `9009` failure, and formal stage prefix propagation. Both scripts parse, the change-impact contract/classification and `git diff --check` pass. No wheel download, candidate, Desktop package, installer, Windows command, CI, R2, signing, installation, update, rollback or publication ran; the main batch sweep retains Windows execution evidence ownership.

## 2026-08-31 · 2.0.15 T25R17 CSS virtual-module source-path isolation

- Baseline and RED: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T25R17-dsh-css-virtual-id`, branch `fix/2.0.15-T25R17-dsh-css-virtual-id`, exact parent `34b8386f1ea4e57b44fd5e342c9e85c5ae7ebe72`. The Windows manifest pollution gate grouped all 50 ASCII developer-path hits into the four CSS-bearing client bundles. A real Rolldown fixture reproduced the shared cause: `dsh-css` embedded the physical stylesheet path in its virtual module id, and Rolldown emitted that id in `//#region`; this is a local build-time boundary, so Sentry is `NO_EVIDENCE`.
- Minimal shared fix and Base identity: Harness commit `32728743c28911bcd4279f79fe9c43ee7aacfb6d` keeps the physical file only in the existing CSS plugin's private map and gives Rolldown a build-root-relative logical id. The same physical path still owns file reads, Lightning CSS filename hashing and watch registration; no component-specific rewrite, pollution-rule exception, dependency, lock or runtime ABI changed. The advertised Harness branch is `fix/2.0.15-T25R17-dsh-css-virtual-id`; the mechanically rebound successor is Base `e-mate-desktop-profile-v15-dsh-32728743c289`, while v14 remains immutable predecessor source evidence.
- GREEN and evidence ceiling: the real generated-chunk regression plus adjacent CSS watch, source-map and bundle-purity contracts pass `15/15`; the parent pin/Base/component/admission checks and four affected bundle builds provide the outer source gates. No candidate, installer, CI, R2, signing, installation, update, rollback or publication runs in this ticket.

## 2026-08-31 · 2.0.15 T09R6 simple native updater closure

- Baseline and Owner audit: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T09R6-simple-native-updater`, branch `fix/2.0.15-T09R6-simple-native-updater`, exact parent `db5083f297ca49732686d2c5bd42fa3245a47648`, Base `e-mate-desktop-profile-v15-dsh-32728743c289`. The Settings action already enters the preload/IPC native bridge, while Agent natural language registers only `e_mate_desktop_update`; both converge on `desktopUpdates.runInteractiveUpdate`, so no second updater or route was needed. The shared checker already returns `update-available` only for a strict higher SemVer and intentionally treats same-version different bytes as `up-to-date`; the downloader already validates exact size, SHA-256 and platform format before atomic completion. This is static/local updater work and `SENTRY_AUTH_TOKEN` is absent, so Sentry is `NO_EVIDENCE`.
- Root fixes and single-install invariant: the missing trust edge was between completed download and native installer handoff. The existing downloader now reopens the atomically landed installer and revalidates its exact bytes, SHA-256 and platform format; the Electron runtime then executes the same signed schema-2 checker again and rejects any failed, no-longer-newer or different-manifest result before staging. The immutable manifest identity therefore still binds the selected landed artifact. The macOS helper now admits only canonical `/Applications/e-Mate.app`, keeps its dot-hidden predecessor solely while commit is unresolved, and deletes it after a healthy durable commit or recovery instead of moving a second `.app` into Trash. The existing Windows transaction already returns to the original install directory before its one registry, Start Menu and desktop-link pass; focused assertions lock that order and uniqueness without changing Windows product code. Root `AGENTS.md` and the existing README release standard now record this same local-only, future-2.0.16+, R2 and single-visible-install contract.
- RED/GREEN and evidence ceiling: focused RED was `7` failures across the post-download trust edge and macOS cleanup, followed by separate canonical-path and landed-reread REDs. Final Desktop updater coverage passes `300/300` across Agent routing, service coordination, strict checker, downloader, Electron handoff, macOS helper and Windows transaction; Desktop source/test typechecks pass, the unchanged Settings bridge test passes `12/12` against its inherited rc.7 test-support closure, and the change-impact contract passes `26/26`. No candidate, exact-Base component build, installer build, real download, installation, replacement, relaunch, Windows host command, CI, R2 write, signing, pointer activation, rollback or publication ran. The same installer bytes' existing installed acceptance is reused only as instructed; real public 2.0.15-to-future-2.0.16+ download evidence remains the release Owner's later gate.

## 2026-09-01 · 2.0.15 T09R7 Windows update mailbox owner

- Baseline and evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-win-mailbox-owner`, branch `fix/2.0.15-windows-mailbox-owner`, exact parent `588c80ccbee572077f47c0ad3a904c8c00cfa004`. The failure occurs before the updated application starts, and Sentry credentials are absent, so Sentry is `NO_EVIDENCE`; the native installer transaction, ACL readback and SSH-driven Windows reproduction are authoritative.
- Root cause: on the built-in Administrator account, a directory created by the real limited interactive installer token inherited `BUILTIN\Administrators` as its owner. The existing mailbox hardening removed inheritance and granted the current user, SYSTEM and Administrators full control, but never transferred ownership; the unchanged strict `Assert-MailboxAcl` therefore rejected its own mailbox before `pending.json` existed. Relaxing the owner assertion or special-casing the installer would leave the shared transaction boundary inconsistent.
- Minimal shared fix: `Set-PrivateDirectoryAcl` now sets the owner of the exact mailbox directory to the already validated current-user SID and checks `icacls` success before applying the existing protected three-principal ACL. The final strict owner, inheritance, allowlist and access-type readback remains unchanged; no fallback, second mailbox, elevation requirement, updater path or installer UI changed.
- RED/GREEN and Windows proof: the focused test was RED because the shared function issued only one `icacls` call, then GREEN `1/1`; the complete owning test file passes `12/12`. Through `ssh win-test-server`, the patched script also passed its native SelfTest and an isolated `RunLevel=Limited`, interactive Administrator Bootstrap probe: both the root and transaction mailbox owners read back as the current user SID, both ACLs were protected, and the exact 2.0.13-to-2.0.15 request was written. The probe removed its pending/mailbox state and task; the installed app remained `2.0.13.0` with zero recovery entries. This is source plus native-transaction evidence only; final installer build, installed replacement/startup, shortcut uniqueness, publication and public readback remain `OPEN` for T18.

## 2026-09-01 · 2.0.15 T09R8 upstream Windows NSIS handoff decision

- Baseline and evidence boundary: the same dedicated worktree and branch continue from `2e9eb12e00c615e712de1e5f31aafd67fa6a4d7c`. The decompression failure occurs inside NSIS before the updated application starts; `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` are unset, so Sentry is `NO_EVIDENCE`. The retained installer error, partial candidate tree and exact path projection are the runtime evidence; this ticket will not touch that remote failure scene.
- Native and historical comparison: the successful 2.0.13 installation record proves only the standard fresh/canonical installer result; it has no private request, journal, phase or path-budget receipt and therefore does not prove the later custom transaction. The user-selected native owner is `anywhere-labs/dsh-desktop@d29bf7a965fc68bf09750bc329905ecb17afe48b`: after verified download it starts the assisted NSIS Setup with exactly `['--updated', '--force-run']`, waits only for successful spawn, then requests orderly application shutdown. e-Mate before `2dcc363a80` likewise used the standard canonical NSIS install path.
- Product decision: by explicit user direction, Windows manual installation and future signed online-update installation now use that one standard assisted NSIS path to replace the original canonical directory and preserve or refresh the sole Desktop and Start Menu shortcut set. Windows no longer promises the custom candidate/journal/ACK health rollback. The signed schema-2 manifest, canonical R2 origin, download progress, landed bytes/size/SHA-256/PE verification and signed-manifest recheck remain mandatory before handoff; 2.0.13 and same-version 2.0.15 continue through the official manual download surface.
- Minimal implementation boundary: remove only the active Windows custom-transaction wiring and restore the pinned upstream process/quit/install seams with bounded e-Mate naming. macOS update behavior, versions, dependencies, R2, publication and remote state remain unchanged; the Yarn lock changes only by its mechanically generated hash/checksum for the existing pinned app-builder 26.15.3 patch. Because this package fixes `nsis.useZip=true`, the upstream 7z extraction optimization is not applicable and was not copied; old-uninstaller code-2 handling also remains absent because no focused current-path failure requires it.
- Root fix and removed path: the runtime now spawns the verified Setup directly with detached/ignored-stdio/no-shell options and requests quit only after the child `spawn` event. The new dedicated `--emate-installer-quit` owner handles both first- and second-instance invocations; `installer.nsh` reuses upstream exact-app graceful shutdown and scoped fallback. The app-builder patch now contains only the existing macOS signing fix plus upstream `ManifestLongPathAware` and exact-executable process matching; standard `nsisunz` extracts directly to canonical `$INSTDIR`. The custom PowerShell coordinator, private runtime module, obsolete tests/SelfTest package script and installSection transaction patch were deleted, leaving one active Windows installation path.
- RED/GREEN and carrier: with pinned Node `/Users/mac/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node` and Vitest 4.1.8, the initial three-file RED failed because the dedicated quit module did not exist and all three new runtime handoff cases observed zero `spawn` calls while the other `43` runtime cases passed. The same focused command is GREEN at `3 files / 51 tests`. Pinned Yarn `/Users/mac/.cache/node/corepack/v1/yarn/4.18.0/yarn.js` changed only the app-builder patch resolution `8a23be → eee6c1`; `install --immutable` and the four-project `typecheck` pass. The full owning set excluding unrelated package fixtures passes `8 files / 164 tests`, and the two exact package/app-builder tests pass `2/2`. A broader `package.spec.ts` run exposed two unrelated existing-closure failures (`redundantEscalation` and `emate/image-output.ignorable`); they are outside this Owner and were not modified.
- Evidence ceiling and risk: source tests prove ordering, negative private-argument contracts, canonical `useZip` extraction, one shortcut pass and the applied dependency patch. No installer was built or run, no SSH/remote scene was touched, and no R2, signing, installation, update, rollback or publication occurred. Real Windows acceptance must still prove assisted UI completion, canonical replacement/relaunch, preservation of the old application on installer failure, and exactly one Desktop/Start Menu shortcut set. Windows intentionally has no custom health rollback after this product decision.

## 2026-09-01 · 2.0.15 T25R21 bounded Windows fuse sharing-lock retry

- Baseline and evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T25R21-windows-fuses-lock`, branch `fix/2.0.15-T25R21-windows-fuses-lock`, exact parent `2bf6c1c5ee8c63ccfd63bce2dde1c9f43debe7d8`, Base `e-mate-desktop-profile-v15-dsh-32728743c289`, Harness `32728743c28911bcd4279f79fe9c43ee7aacfb6d`, and pinned Desktop reference `6074088f5b660206e404b3591fab51fb99c69add`. On authorized Windows host `172_16_48_13`, exact source passed all `267/267` Desktop tests, including `e-mate-profile-win` in `17.57s`; two fresh `dist\win-unpacked\e-Mate.exe` packaging attempts then failed at the sole `app-builder-lib` `addElectronFuses → @electron/fuses flipFuses` seam with `UNKNOWN: unknown error, open`, while post-exit exclusive ReadWrite open succeeded for the `225533440`-byte file and Defender reported no threat/quarantine. This is a pre-runtime Base packaging boundary, so Creation Mode and Sentry are inapplicable.
- Root-cause and native comparison before implementation: the evidence identifies a transient Windows sharing lock on the just-extracted executable, not a fuse-format or application failure. Pinned Desktop reference `6074088` retains direct fuse execution and adds no alternate packaging owner; e-Mate's pinned `app-builder-lib@26.15.3` already depends on and imports `builder-util-runtime@9.7.0`, whose exported `retry(task, { retries, interval, backoff, shouldRetry })` is the existing bounded retry seam. The fix remains limited to the existing Yarn patch; Defender, fuse execution and all later verification remain mandatory.
- Minimal shared fix and ceiling: the existing `app-builder-lib@26.15.3` Yarn patch now wraps only the Windows `flipFuses` call in the existing runtime retry helper. A retry requires the exact observed `code === UNKNOWN`, `syscall === open`, and an error path resolving case-insensitively to that same Electron executable; non-Windows calls and every other error still execute/fail once. The ceiling is three retries after the initial attempt, delayed `1s + 2s + 3s` for at most `6s` total wait; the fourth matching failure is final. No Defender setting, fuse option, verification, dependency version, alternate executable or continue-on-error path changed.
- RED/GREEN and evidence ceiling: the standalone regression was RED against the exact baseline installed graph (`1/3`, target recovery and ceiling failed), then GREEN `3/3` after the patch, proving transient recovery, immediate non-target `EACCES` failure and the four-attempt ceiling. Pinned Yarn `4.18.0` install updated only the expected patch hash/checksum; `install --immutable` passes. A pristine `app-builder-lib@26.15.3` extraction passes full `git apply --check`, actual replay and installed-code readback. The focused installed patch contract passes `1/1`, adjacent NSIS contract `3/3`, change-impact contract `26/26`, and `git diff --check` passes. No candidate, installer, Windows build, Defender change, CI, signing, R2 operation, installation, update, rollback or publication ran. The main release Owner must rerun the request-owned exact Windows `_platform-build` on host `172_16_48_13` and require fuse completion plus the unchanged installer/result verification before advancing candidate truth.

## 2026-09-01 · 2.0.15 T07R4 Skill Hub versioned result schema

- Baseline and installed evidence: dedicated clean worktree `/Users/mac/e-mate/worktrees/emate-2.0.15-T07R4-skill-hub-result-schema`, branch `fix/2.0.15-T07R4-skill-hub-result-schema`, exact parent `2bf6c1c5ee8c63ccfd63bce2dde1c9f43debe7d8`, Base `e-mate-desktop-profile-v15-dsh-32728743c289`. The installed r14 application is 2.0.15 with executable SHA-256 `d1e9071d5e62f9ea838d57381f180d8a04555291d30d6fc4d84266652892be2f`, app.asar SHA-256 `a081ff96fc2b369575ca7105dc3e7939470486dc1bb54b94d7ec062cda65c304`, deep strict codesign PASS, bundled Profile active, 64 sessions retained, and one installed Skill still visible. Its capability-center catalog path exposed the rc.7 Zod `invalid_union`: success expected `ok: true`, failure rejected the custom `error.code`, then the response `result` failed. Direct loopback replay against `/emate.skillHub/catalog.search` also captured the installed producer throwing through the carrier as HTTP 500 `handler failure: TypeError: fetch failed`; extracting the exact installed asar proved that handler awaited `hub.search` with no terminal catch.
- Unique Owner and pinned seam: `@e-mate/dsh-plugin-skill-hub` remains the sole Profile component Owner from authenticated Hub client through `/emate.skillHub`, local rc.7 Skill provider, Jobs and capability-center projection. The exact Base-bound rc.7 `@deepseek-ai/dsh-client-connection` types require handlers to return `RpcResult`; `@deepseek-ai/dsh-host-apiproxy` closes that failure discriminator to its own core codes. The current source's attempted catch still placed Hub-only `auth/network/conflict/integrity/recovery/native-provider` codes in that closed core slot, reproducing the observed parser failure whenever a typed Hub failure occurred. Creation Mode and `cordis_inspect_*` tools were unavailable in this delegated session, so the seam was established from exact pinned commit `32728743c28911bcd4279f79fe9c43ee7aacfb6d` package types/runtime schema plus the exact installed asar and loopback response; no seam was guessed.
- Minimal root fix: one shared schema-version 1 module now owns exact `success` / `failure` Hub results, its closed Hub discriminator set, fixed user-safe messages, producer constructors and client parser. The Host always returns a valid rc.7 transport success whose value is that Hub result; malformed input and caught service/native failures use the Hub failure branch instead of occupying rc.7's core error slot. The Client validates the same exact keys/version/discriminator before presentation and maps malformed transport, the real installed bad payload and unknown future discriminators to one bounded `invalid-response` result. Job failure detail uses the same fixed mapping. No browser string filter, retry, second transport, alternate provider or second lifecycle path was added.
- Regression and gates: the component build passes; Host/component tests pass `19/19`, including the real installed bad payload, unknown discriminator, normal installed inventory and captured Host failure payload; client tests pass `10/10` against exact pinned source aliases. `check:target` passes; component release/ABI contracts pass `14/14`; change-impact contracts pass `26/26`; focused emitted-component ABI generation succeeds with only the declared Base imports. The ordinary combined component test command first exposed a clean-worktree-only missing client test-runtime and then duplicate-React resolution after the Harness dependency install's root lefthook step rejected nested-worktree Git config; the same owning tests passed after building the exact pinned Host/Client libraries and running the client suite with a temporary exact-source alias config. No application, installer, Profile generation, remote service, R2 object, pointer or publication was modified.
- Evidence ceiling and recheck: this commit is `source fixed`, not installed accepted or released. The main acceptance Owner must compose this exact committed component into the next candidate, replace the exact installed app through the established candidate flow, verify the installed hashes/signature, open Capability Center with the retained 64-session state, and confirm catalog failure is bounded Chinese copy with no Zod text while the installed Skill list remains visible; then refresh, switch Discover/Installed and restart before closing installed acceptance.

## 2026-09-01 · 2.0.15 r15 Profile snapshot candidate identity refresh

- Baseline and failure: clean r15 integrated Windows fuse and Skill Hub commits over `2bf6c1c5ee`. The authoritative `flow dev` passed `81/82` and failed only the checked-in current desired-state snapshot because its candidate metadata still named Base v14 while `desktop/e-mate-desktop/base-contract.json` freezes Base v15. The three captured target byte sets themselves still decode to the exact public 2.0.13 Base v7 desired states and retain their original bytes and SHA-256 values.
- Root fix: refresh only `candidate_base_contract_id` to `e-mate-desktop-profile-v15-dsh-32728743c289` and recompute the enclosing canonical `snapshot_sha256` with the existing `canonicalProfileJson` owner. Capture authority, source origin, timestamp, target envelopes, signatures and public predecessor identities remain byte-for-byte unchanged; no Cloudflare read or write is fabricated because the candidate-only identity is repository state, not captured R2 content.
- Evidence ceiling: the focused checked-in snapshot gate passes after the refresh. This is release-control source evidence only; the committed r15 SHA must still rerun `flow dev`, build one new exact candidate and pass installed acceptance before any R2 publication. No installer, R2 object, pointer, website or installed application changed in this slice.
