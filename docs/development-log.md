...
- Verification commands and results: 新快照 `43255` bytes / SHA-256 body identity `bdbcd39f1e04e60118f4d93cb25e4c9b7d887c1d02c2a6ba9917069c8ecd51bc`；现有 materializer 成功输出三目标 `10579/10567/10503` bytes，Profile publication tests `2/2` 通过。
- Immutable evidence / receipt: Cloudflare 插件返回三对象均为 `application/json`、`no-store`，generation 分别为 `8f13e65c…3822`、`569c5fe5…c72e`、`c363cee9…deaf`；失败 run 的精确错误为 `Profile current desired-state snapshot is invalid`，且没有 publication artifact 或线上写入。
