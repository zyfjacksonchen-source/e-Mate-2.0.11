...
Browser capability is fail-closed and platform-specific. Windows selects exact Microsoft `@playwright/mcp@0.0.78` with the already-installed system Edge and forbids browser installation or secondary download. Because rc.5 workspace-root/permission composition and real Windows acceptance are incomplete, its readiness is `PLAYWRIGHT_MCP_EDGE_UNVERIFIED`/`setup-required`. macOS Ego Browser is also setup-required until real launch, permissions, session isolation, process cleanup and interaction/download evidence pass. `browser-panel` only projects an accepted adapter through target Connection RPC and slots; it cannot make an unverified adapter ready.
