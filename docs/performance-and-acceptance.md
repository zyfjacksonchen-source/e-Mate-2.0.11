...
The protected environment provides `EMATE_PERFORMANCE_COLLECTOR` and its exact `EMATE_PERFORMANCE_COLLECTOR_SHA256`. The owner separately resolves the protected-main `scripts/performance-acceptance-probe.mjs`, hashes both regular canonical files, and proceeds only when the protected source digest, configured digest and runner-installed digest are identical; both source and installed digests are bound into the private plan. A runner-local hash without those reviewed source bytes is not provenance. The owner executes the installed copy once with the workflow's pinned Node 24 as `node <collector> --plan <private-plan.json>`, with no shell and a credential-free environment allowlist; this is the same path on macOS and Windows and does not rely on file associations. The plan is schema 1, mode `production-installed-performance-acceptance`, and closes source commit, both Harness pins, collector provenance, exact downloaded candidate root, the already-verified Profile publication root plus its acceptance receipt/aggregate paths, scratch root, four public model objects, four unique run ids and every row's pair/scenario/AB-BA/path order. It contains no prompt body or credential. The probe's reviewed bytes own the fixed acceptance dataset and must use the installed app's existing login/lease and native transports rather than a hidden API key.

The version-controlled Node 24 probe is acceptance-only. Its config is fixed outside workflow env/plan at an owner-only canonical OS path and may contain only keychain/local-secret-broker references for the dedicated acceptance identity and admin joined exporter, never credential values. Its CDP bootstrap records trusted send, first non-empty assistant chunk, current-turn first text and double-`requestAnimationFrame` paint on the renderer's single `performance.now()` clock without retaining text. Usage joins require exact dedicated account/model/cursor/scope plus request/invocation/response ids; extra, missing or ambiguous rows fail, and raw ids are domain-separated hashed before return. Offline-valid-cache may cut only auth/policy control plane while the inference gateway remains available and unchanged lease/policy receipts remain inside their refresh windows. The current pinned Desktop has no external read-only native Session/request/Tool exporter, so the checked-in probe deliberately exits non-zero after preflight rather than manufacture the 17 sources. Production collection remains OPEN until that existing native authority is exposed to the runner and implemented directly here; a private diagnostic, DOM scrape, time-window join or fixture cannot close it.
