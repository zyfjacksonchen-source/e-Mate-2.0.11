...
The production signer accepts exactly four attempt-1 evidence artifacts named with the short route ids `luna`, `sol`, `deepseek` and `doubao`. Each contains exactly one `performance_model` object matching the fixed public route/provider/model/reasoning roster, one evidence JSON, the protected verifier, the recomputed Profile aggregate and the same 20 referenced receipts. It emits four independently signed leaf admissions under `children/01-luna` through `children/04-doubao`, then one domain-separated outer `emate.performance-aggregate-admission`. The outer `ttft-v2-aggregate` verifier binds the ordered leaf verifier decisions; its aggregate run id alone enters the existing four-field Desktop performance summary. Missing, extra, reordered, duplicated, rerun, fixture or mixed-model evidence fails closed.
