...
This document defines optional performance diagnostics and non-performance acceptance checks for e-Mate 2.0.13. Performance/TTFT/startup/FPS/heap results do not gate Desktop/Profile admission, signing, coordinator, publication, website activation, update, or rollback. When diagnostics are run, a scenario is `passed` only with commands, screenshots or traces, immutable IDs, and the observed result; missing authorities remain `blocked` diagnostics and mocks cannot be relabelled as production evidence.
...
The optional diagnostic signer accepts exactly four attempt-1 evidence artifacts named with the short route ids `luna`, `sol`, `deepseek` and `doubao`. Each contains exactly one closed `performance_model` object matching the fixed public route/provider/model/reasoning roster; the assembler and verifier must preserve it byte-for-byte and every path receipt must name the same provider/model/reasoning tuple. Each leaf also contains one evidence JSON, the protected verifier, the recomputed Profile aggregate and the same 20 referenced receipts. It emits four independently signed leaf diagnostics under `children/01-luna` through `children/04-doubao`, then one domain-separated outer `emate.performance-aggregate-admission`. Missing, extra, reordered, duplicated, rerun, fixture or mixed-model evidence fails that diagnostic; no aggregate identity enters the Desktop manifest or release-state.
...
The version-controlled Node 24 probe is acceptance-only. On macOS its config is fixed at the current runner user's owner-only `~/Library/Application Support/e-Mate/acceptance/performance-probe.json`; it contains only opaque identity/exporter/offline-control references and one owner-only installation root, never credential values. Each reference resolves to an owner-matched canonical `0700` executable under the sibling `brokers/` directory. The probe invokes it without a shell, with bounded canonical JSON stdin/stdout and no secret-bearing environment: `usage-snapshot` returns the complete paginated sanitized ledger for one account/model/provider, while `auth-available`, `auth-unavailable` and `status` control only the acceptance auth/policy endpoint. The installation root's frozen `baseline/` contains exactly the separately provisioned `e-Mate.app`, `installed-runtime.json`, `base-contract.json`, `component-inventory.json`, and `profile-generations/{state.json,store/}` authorities; the workflow never reads or copies a user's live application state. The built-in Darwin lane verifies that 2.0.12 application plus its Base contract, component inventory, exact non-bundled Profile state and content-addressed store; it copies only the natively verified release/component closure into each isolated `userData`. The historical Base v6 file is admitted only at its exact byte digest, id, Harness and Desktop-reference identities; because that frozen file, release and component manifests predate `schedule_protocol_floor`, the probe creates an in-memory floor-0 compatibility view solely for native verification. The normal Base parser still rejects a missing or zero floor, and no other legacy Base or manifest receives this path. The candidate uses the already-downloaded signed publication bundle as an offline request source and the native Profile assembler/verifier to materialize its exact accepted generation; it cannot fetch missing bytes. After every launch the probe reads the actual `profile-generations/state.json`, fully re-verifies the physical generation, manifests and files, and reads the generated DSH `.e-mate-install.json`; `active` and `last_known_good` must both equal the expected generation, with no symlink, escaping path or `bundled` fallback. It also verifies and read-only mounts the exact candidate DMG, copies it to a runner-private same-volume directory, checks executable/architecture/signature bindings, injects CDP before reload, and always stops, detaches and cleans up. The CDP bootstrap records trusted send, exact string `/api/events.mux` assistant chunk, current streaming assistant text and double-`requestAnimationFrame` paint on one renderer clock without retaining text. Usage joining uses complete before/after ledger ID sets under a dedicated single-flight account, and every identifier hash includes `performance_run_id`. Missing or ambiguous fields fail the owned sample; a private diagnostic, DOM text scan, time window or fixture cannot close production evidence.
...
If the optional four-model diagnostic is dispatched, its aggregate and all child run ids remain distinct and signed inside that diagnostic artifact. A missing, fixture, superseded, mismatched or semantically incomplete child fails that diagnostic only. Desktop/Profile release manifests and release-state must not contain its run id, receipt, summary, signature, pass flag, or a fabricated substitute.
