...
| Terminal audit scenario | T10R2 proves terminal-only trusted provenance, lifecycle-wide lock, privacy, restart replay and per-session isolation | T18 runs model-only, Office, ImageGen and Search on exact installed bytes, then reconciles local outbox, service receipt, Postgres aggregation and Dashboard; offline audit never delays the local result |
| Project session and memory | T19 proves one native create/open owner, project/session binding and draft/attachment isolation | T18 covers first creation, retry, refresh, restart, cross-project/cross-session negative cases and durable project-memory recall |
| Home, native `@`, and Composer frame | T21 plus T16R8 prove one semantic host, Goal/Plan native actions, fixed five-source roster and no ancestor guessing | T18 captures Home and active conversation at required widths/scales/themes; glow boundary matches Workspace+InputBar within 2px and Settings/Capability routes retain their own chrome |
| Current-Turn artifacts | T26 proves `callId`/latest-`revision` selection, child promotion, typed file/image actions, privacy and deduplication | T18 runs single/multi/subagent/partial/hung/restart cases and verifies every successful image/file becomes usable immediately without waiting for unrelated children |
| Local release transaction | T20/T25 prove pinned Corepack/Node/npm carriers, one candidate, exact Windows Remote import, validated R2 Owner receipts and fail-closed rollback ledger | T18 builds once, binds both installers and installed receipts to one run, signs the manifest, activates only through Cloudflare R2 CAS, exercises failed-health rollback and publicly rereads exact bytes |
