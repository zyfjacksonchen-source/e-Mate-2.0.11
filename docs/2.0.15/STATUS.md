...
| Tencent backlog | `INVESTIGATED` | T17 has one authoritative input only: functional-feedback export `VwnSnZLbANDI`; the user declared the separate image-feedback workbook obsolete old-version material, so its content is excluded rather than imported or mapped |
...
| T16 | Product plugins, GenUI and Glass Composer contract | 5.6 sol max / high | `feat/2.0.15-T16-product-components` | T03 audit; T05 routing; T11 host; T12 gallery contract | `worktrees/emate-2.0.15-T16` | `INVESTIGATED` | visible task `01a0437a-bff4-7db2-9387-9edbe2b01e1d`; D012/D013 native host is merged and the same task may resume after branch synchronization |
| T17 | Tencent backlog intake | 5.6 sol / medium | `feat/2.0.15-T17-tencent-backlog` | readable Tencent sheet; T03 audit | `worktrees/emate-2.0.15-T17` | `INVESTIGATED` | visible task `01a04376-18e2-7a63-9c79-934597ccae80`; only `VwnSnZLbANDI` is authoritative; obsolete image-feedback content is explicitly excluded and prior mixed uncommitted output must be replaced |
