...
| Base/Harness/Desktop pins | `BLOCKED` | immutable v7 remains unchanged; T13 now pins final local Harness `4787caf391…`, so T18 must bind that gitlink in the honest successor Base before RC |
| Harness source reachability | `BLOCKED` | `4787caf391…` is locally available for integration, but the first advertised-origin submodule fetch returned `not our ref`; T18 must prove a clean checkout can fetch the exact gitlink before RC |
