...
- First retry comparison found a P1: two persisted `llm/retry` events with one `retryId` were correctly present in the target projection, but the target default Retry View rendered only `data.current`. The scoped repair shadows only the official `conversation.chat.node / model-retry` keyed slot and renders the target-owned `attempts`. The main-agent rerun shows “上次尝试失败 / 第 1 次” and “重试已取消 / 第 2 次”; opening both rows reveals their distinct delays and failure reasons. The post-fix client bundle SHA-256 is `c6ef8c8f844a7f57128e906f196d46445bbad29a159e33bfdb6bfcec2b01f20a`; combined evidence is `source-vs-current-retry-after-1280.png`.
