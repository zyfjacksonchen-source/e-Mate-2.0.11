...
## S02/S03 Codex-like message-flow replacement — 2026-08-17

- Visual source: the user-approved isolated prototype in `/Users/mac/Documents/Codex/2026-08-16/jia/outputs/deepseek-harness-codexlike-demo/qa/`, specifically `error-collapsed-1440x1000.png`, `final-text-shimmer-1440x1000.png`, `image-final-gallery-1440x1000.png`, and `image-final-gallery-narrow-390x844.png`. Implementation evidence uses the current e-Mate DSH shell, real target JSONL Session persistence and real CAS image attachments; no static replacement renderer or backend mock is present in the product diff.
- Same-canvas comparisons are `artifacts/design-qa/S02-codexlike-message-flow/source-vs-implementation-terminal-1440x1000.png`, `source-vs-implementation-expanded-1440x1000.png`, `source-vs-implementation-gallery-1440x1000.png`, and `source-vs-implementation-gallery-mobile-390x844.png`. Full implementation states, including the existing target lightbox, are stored beside them. Desktop was checked at `1440×1000`; responsive layout was checked at `390×844`, DPR 1.
- Layout and hierarchy: successful/max-token turns expose one terminal activity Header and default to collapsed; running activity remains expanded; failed, blocked, cancelled and interrupted Headers are non-disclosure status rows. Reasoning and non-error Tool/Command nodes are the only collapsible members. Final Assistant Markdown, message actions, retry/errors, ProducedFiles and attachments remain outside the collapse boundary.
- Typography, surfaces and motion: the production result keeps e-Mate's existing 14/22 message scale, 13/20 tool scale, dark tokens and 44px interaction targets while adopting the prototype's quieter Header rhythm. The 1.45s sweep is clipped to running text glyphs only; no row or card receives an animated overlay. `prefers-reduced-motion` removes the sweep and Chevron movement. The existing Domino component remains the running indicator and is hosted by the real activity Header without an OpenAI mark.
- Media fidelity: final Assistant images continue through the existing target `ImageGallery` and `ImageLightbox`. The real two-image fixture expands from `已查看 2 张图像`, keeps equal target tiles, opens one named dialog, closes and collapses back to the same DOM. `image-gallery.tsx` and the composer are unchanged.
- Responsive and accessibility checks: the mobile captures show no document-level horizontal overflow or control collision. Activity controls are native buttons with stable `aria-controls` and accurate `aria-expanded`; terminal success collapse/expand, image disclosure, lightbox close and copy feedback were exercised. The real browser had zero error/warn entries after a fresh load. Shell coverage includes the running Header/host, terminal collapse boundary, error visibility, target gallery reuse and reduced-motion/style contracts.
- Comparison history: the first production pass exposed member-ID churn during disclosure toggles. The activity observer now owns stable generated IDs for its lifetime and uses a ref for expansion updates, avoiding observer teardown/re-observation on each click. The final Browser replay preserved the exact `aria-controls` value through closed → open → closed. No remaining P0, P1 or P2 visual defect was found in this scoped replacement.

final result: passed (Codex-like activity hierarchy, text-only shimmer, terminal/error states and existing image gallery)
