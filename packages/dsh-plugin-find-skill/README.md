...
The official `skills` CLI is pinned to `1.5.22` and runs through Desktop's packaged `pnpm`; `latest` is never resolved during a 2.0.9 run. The four e-Mate connector Skills resolve from the immutable `skills-v2.0.9` Git tag.
