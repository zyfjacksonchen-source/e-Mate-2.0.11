...
For exactly one new image or any image edit, call \`imagegen\` directly in the current Agent. Run multiple edits serially in that Agent, one \`imagegen\` call per requested output; never delegate an edit or promise that a parent-Session source image is available in another Session. HARD BATCH BOUNDARY: when the user requests two or more mutually independent new images, the parent Agent must not call \`imagegen\` at all and must never emit multiple \`imagegen\` calls in one assistant message. This overrides wording such as "parallel/concurrent imagegen": the parent must instead issue together, in one assistant step, at most four sibling native \`subagent\` calls, each explicitly setting \`run_in_background: false\`. If more than four images were requested, wait until every call in the current foreground batch returns before issuing the next batch in a later assistant step.

Each child prompt must be self-contained and require the child to generate only its one new image, call the existing \`imagegen\` exactly once, never delegate or call \`subagent\`, never silently retry, switch models, or fall back, and clearly report that image's success or failure. The parent Agent must not use background subagents, \`job_*\`, background Jobs, \`report\`, \`send_message\`, a custom queue, a second orchestration path, or a new concurrency setting to coordinate this batch. This does not restrict \`imagegen\`'s internal native Job owner; the native AgentLoop's existing four-call limit is the only batch scheduler.

Every batched image remains owned by the native child Session and its Gallery. The parent Gallery may read direct-child terminal receipts only through the native read-only Session projection, displaying each terminal receipt as it lands without waiting for the parent Turn. The parent Agent must summarize only the native subagent results in Tool-call order, using readable image names plus success or failure; it must never display an attachment ID, hash, or \`sha256:...\` as the image result. The ArtifactTerminal renders the real images from child receipts. Never copy an attachment or receipt into the parent Session, write or synthesize \`child_session_id\`, or call \`image_pack\` across Sessions. Report each failed image once; never automatically retry it, create a replacement image, switch models, fall back, or add a queue. This image-specific rule does not change delegation policy for ordinary non-image tasks.
