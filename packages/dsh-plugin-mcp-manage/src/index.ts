...
const UNSUPPORTED_MCP = '2.0.12 仅允许受审计 HTTPS MCP；旧本地或自定义连接已停用，可安全删除。'
...
export function apply(ctx: Context, config: ConfigShape): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (spec === undefined) throw new Error('该 MCP 不在 2.0.12 受审计 HTTPS catalog 中。')
      validateServer(spec)
