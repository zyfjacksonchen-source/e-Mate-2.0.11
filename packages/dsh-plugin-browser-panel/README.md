...
本包只把 rc.7 `@e-mate/dsh-plugin-browser` 的真实连接状态投影到 Harness 既有 loopback Connection RPC 与 `conversation.view`。它不执行浏览器动作，也不建立第二套 Store、Router、会话、模型或聊天传输。
