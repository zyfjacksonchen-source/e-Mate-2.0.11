...
The adapter retains only bounded memory records and copy-on-write import semantics. Its storage and execution are normalized onto DeepSeek Harness `0.1.0-rc.7` public `WorkspaceRegistry`, `storageDomain`, `tools`, and `systemPrompt` services. No upstream UI, HTTP/WebSocket transport, updater, model configuration, COI, session orchestration, global/user memory, sync, dream distillation, or autonomous-learning code is included.
