...
await writeFile(join(root, 'lib/emate-capability.js'), `export function installComputerUseCapability(ctx, service) {
    return ctx.effect(() => ctx.emateCapabilities.register({
        ...
        status: async () => {
            ...
            return action_ids.length === 0
                ? { state: 'ready', detail: '原生 helper、辅助功能和屏幕录制权限均已就绪。', action_ids }
                : { state: 'setup-required', detail: '需要在 macOS 系统设置中开启对应权限。', action_ids };
        },
