...
At startup, e-Mate launches the already-installed Google Chrome with remote debugging bound to loopback and a persistent isolated profile under `$DSH_HOME/runtime/cdp-chrome`. This satisfies current Chrome's non-default-profile requirement without an extension, developer-mode loading, a downloaded browser, or a manual remote-debugging toggle. The e-Mate Profile enables its independent CDP control grant by default, so the Agent can immediately use `browser_tabs` or `browser_snapshot`. The user can disable that grant in the capability center. Full Access remains a separate filesystem policy and never bypasses the CDP plugin's own control grant.
